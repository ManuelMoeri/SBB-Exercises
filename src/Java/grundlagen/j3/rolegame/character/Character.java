...
import Java.grundlagen.j3.rolegame.Fight;
import Java.grundlagen.j3.rolegame.item.Item;
import Java.grundlagen.j3.rolegame.RolegameInstances;
...
public class Character {
    ...
    public void takeArmor(Armor armorToTake) {
        if (armorToTake.getArmorWeight() <= this.carryingCapacity) {
            ...
            if (armorToTake.equals(RolegameInstances.LIGHT_ARMOR)) {
                setInitiativeValue(getInitiativeValue() - 2);
            } else if (armorToTake.equals(RolegameInstances.HEAVY_ARMOR)) {
    ...
    public void addWeaponToInventory(Weapon weaponToAdd) {
        RolegameInstances.activeCharacters.get(Fight.playerNumberInList).takeWeapon(weaponToAdd);
    }

    public void addItemToInventory(Item itemToAdd1, Item itemToAdd2) {
        RolegameInstances.activeCharacters.get(Fight.playerNumberInList).takeItem(itemToAdd1);
        RolegameInstances.activeCharacters.get(Fight.playerNumberInList).takeItem(itemToAdd2);
    }
