...
public class StringExam1 {
    ...
    public static void CountWords(String endOfTheWorld) {
        endOfTheWorld = endOfTheWorld.toLowerCase();

        String[] splittedArray = endOfTheWorld.split(" ");
        String emptyArray = " ";
        ...
        for (int i = 0; i < splittedArray.length; ++i) {
            String buffer = "";
            for (int j = i+1; j < splittedArray.length; ++j) {
                buffer = splittedArray[i];
                if (buffer.equals(splittedArray[j])) {
                    counter++;

                    for (int k = 0; k < splittedArray.length; k++) {
                        if (splittedArray[k].equals(splittedArray[j])) {
                            splittedArray[k] = "mhm";
                        }
                    }
                }
