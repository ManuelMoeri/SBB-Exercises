...
public class StringExam5 {
    public static void MostUsedWords(String endOfTheWorld) {
        String[] firstarray = endOfTheWorld.split(" ");
        String[] secondarray = endOfTheWorld.split(" ");
        ...
        for (int i = 0; i <= firstarray.length - 1; i++) {
            for (int j = 0; j <= firstarray.length - 1; j++) {
                buffer = firstarray[i];
                if (buffer.equals(secondarray[j])) {
                    counter++;
                }
            }
            if (counter > result) {
                result = counter;
                counter = 0;
                MostUsedWord = firstarray[i];
            } else {
                counter = 0;
            }
        }
        System.out.println("The word " + "\"" + MostUsedWord + "\"" + " is the most common word! There are " + result + " instances of this word!");
    }
    ...
    public static void main(String[] args) {
        ...
        endOfTheWorld = endOfTheWorld.replace(",", "");
        endOfTheWorld = endOfTheWorld.replace("(", "");
        endOfTheWorld = endOfTheWorld.replace(")", "");
        endOfTheWorld = endOfTheWorld.replace(".", "");
        endOfTheWorld = endOfTheWorld.replace("\"", "");
